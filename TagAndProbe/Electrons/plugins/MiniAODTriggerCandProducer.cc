...
bool MiniAODTriggerCandProducer<pat::Electron>::onlineOfflineMatching(const edm::TriggerNames & triggerNames, 
								      ...
								      pat::ElectronRef ref, std::string filterLabel, float dRmin, bool isFilter ) {

  for (pat::TriggerObjectStandAlone obj : *triggerObjects) { 
    obj.unpackPathNames(triggerNames); 
    if ( (isFilter && obj.hasFilterLabel(filterLabel)) || (!isFilter && obj.hasPathName(filterLabel,true,true)) ) {
    //if( obj.hasPathName( filterLabel, true , true ) ){
...
bool MiniAODTriggerCandProducer<pat::Photon>::onlineOfflineMatching(const edm::TriggerNames & triggerNames, 
								    edm::Handle<pat::TriggerObjectStandAloneCollection> triggerObjects, 
								    pat::PhotonRef ref, std::string filterLabel, float dRmin, bool isFilter) {
...
bool MiniAODTriggerCandProducer<reco::RecoEcalCandidate>::onlineOfflineMatching(const edm::TriggerNames & triggerNames, 
										edm::Handle<pat::TriggerObjectStandAloneCollection> triggerObjects, 
										edm::Ref<std::vector<reco::RecoEcalCandidate>> ref, std::string filterLabel, float dRmin, bool isFilter) {
